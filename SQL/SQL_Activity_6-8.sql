...
SELECT customer_id , MAX (purchase_amount) AS Highest_Purchase FROM orders GROUP BY customer_id ORDER BY customer_id;

-- Write an SQL statement to find the highest purchase amount on '2012-08-17' for each salesman with their ID.
SELECT salesman_id, order_date, MAX (purchase_amount) AS Highest_Purchase FROM orders WHERE order_date =TO_DATE('2012-08-17', 'YYYY-MM-DD') GROUP BY salesman_id,order_date;
...
SELECT customer_id, order_date, MAX (purchase_amount) AS Highest_Purchase FROM orders WHERE purchase_amount = 2030 OR purchase_amount = 3450 OR purchase_amount = 5760 OR purchase_amount = 6000 GROUP BY customer_id,order_date;

SELECT customer_id, order_date, MAX(purchase_amount) AS Highest_Purchase FROM orders  GROUP BY customer_id,order_date HAVING Highest_Purchase IN (2030,3450,5760,6000) ORDER BY customer_id ;